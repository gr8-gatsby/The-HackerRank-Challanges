...
for _ in range(n):
    ...
    name_adr = elem.split()
    adr = (name_adr[1].rstrip('>')).lstrip('<')

    try:
        name = re.split("@", adr, 1)[0]
        rest = re.split("@", adr, 1)[1]
        dom = rest[::-1].split('.', 1)[1][::-1]
        ext = rest[::-1].split('.', 1)[0][::-1]

        x = re.search("^[a-zA-Z][a-zA-Z0-9-_\.]*$", name)
        y = re.search("^[a-zA-Z]+$", dom)
        w = re.search("^[a-zA-Z]+$", ext)
        z = len(ext)

        if x and dom.isalpha() and ext.isalpha() and 4 > z > 0:
            print(elem)
